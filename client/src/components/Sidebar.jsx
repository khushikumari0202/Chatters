    ...
    text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
        <div className='pb-5'>
            ...
            <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
                ...
                <input type='text' placeholder='Search User...' className='bg-transparent border-none
        ...
        <div className='flex flex-col'>
            {userDummyData.map((user, index) => (
                ...
                max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]'}`}>
                    ...
                    {index > 2 && <p className='absolute top-4 right-4 text-sm h-5
                    w-5 flex justify-center items-center rounded-full bg-violet-400'>{index}</p>}
                </div>
            ))}
        </div>
    </div>
